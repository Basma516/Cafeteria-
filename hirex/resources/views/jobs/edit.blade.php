...
<div class="site-section">
    ...
    @csrf
    @method('PUT') <!-- This will generate a PUT request -->
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror">{{ old('description', $job->description) }}</textarea>
                @error('description')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="requirements" id="requirements" class="form-control @error('requirements') is-invalid @enderror">{{ old('requirements', $job->requirements) }}</textarea>
                @error('requirements')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select name="job_status" id="job_status" class="form-control @error('job_status') is-invalid @enderror">
                    ...
                    @foreach ($statuses as $status)
                        <option value="{{ $status->id }}" {{ (old('job_status', $job->job_status) == $status->id) ? 'selected' : '' }}>
                            {{ $status->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="job_type" id="job_type" class="form-control @error('job_type') is-invalid @enderror">
                    ...
                    @foreach ($jobTypes as $type)
                        <option value="{{ $type->id }}" {{ (old('job_type', $job->job_type) == $type->id) ? 'selected' : '' }}>
                            {{ $type->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea name="responsibilities" id="responsibilities" class="form-control @error('responsibilities') is-invalid @enderror">{{ old('responsibilities', $job->responsibilities) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea name="benefits" id="benefits" class="form-control @error('benefits') is-invalid @enderror">{{ old('benefits', $job->benefits) }}</textarea>
                @error('benefits')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="logo" id="logo" class="form-control @error('logo') is-invalid @enderror" value="{{ old('logo', $job->logo) }}">
                @error('logo')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group mt-3">
                <button class="btn btn-dark" type="submit" style="background-color: #5289b5; border-color: #5289b5;">Update Job</button>
            </div>
        </div>
    </div>
</form>
                ...
                <form action="{{ route('jobs.destroy', $job->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this job?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger mt-3">Delete Job</button>
                </form>
            </div>
        </div>
    </div>
